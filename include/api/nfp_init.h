...
typedef struct nfp_initialize_param_t {
	...
	/**
	 * MTU to set on interfaces. The actual value is adjusted based on NIC
	 * capabilities and packet pool settings.
	 *
	 ...
	 */
	struct arp_s {
		/** Maximum number of ARP entries. Default is NFP_ARP_ENTRIES. */
		int entries;

		/** ARP hash bits. Default is NFP_ARP_HASH_BITS. */
		int hash_bits;

		/** Entry timeout in seconds. Default is NFP_ARP_ENTRY_TIMEOUT. */
		int entry_timeout;
	...
	/**
	 * Maximum number of VLANs. Default is NFP_NUM_VLAN.
	 */
	int num_vlan;
	 ...
	 */
	struct mtrie_s {
		/** Number of routes. Default is NFP_ROUTES. */
		int routes;
		/** Number of 8 bit mtrie nodes. Default is NFP_MTRIE_TABLE8_NODES. */
		int table8_nodes;
	} mtrie;
