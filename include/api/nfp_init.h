...
typedef struct nfp_initialize_param_t {
	...
	/**
	 * Global NDP parameters.
	 */
	struct ndp_s {
		/**
		 * Maximum number of concurent NDP timeout operations.
		 * NDP timeout operations include neighbor solicitation
		 * retransmissions and prefixes and routers expirations.
		 * Default value is NFP_NDP_TIMEOUT_OPS_ENTRIES.
		 */
		int timeout_entries;

		/**
		 * Time interval (in seconds) between NDP timeout operations
		 * status checks.
	...
	*/
	...
	struct dhcp_s {
		...
		*/
		...
		/**
		 * DHCP Unique Identifier (DUID) type.
		 * Default value is NFP_DH6_DUID_TYPE_LLT.
		*/
		uint32_t duid6_type;
	} dhcp;
