...
typedef struct nfp_ifport_net_param_t {
	 ...
	 *  Valid only if NFP_POWER mode is enabled at build time.
	 */
	odp_bool_t intr_mode;
...
/**
 * Configure IPv6 address on a network port interface with dhcp
 *
 * The function applies to the interface associated to the port (when
 * subport_vlan is NFP_IFPORT_NET_SUBPORT_ITF) or to a VLAN.
 * VLAN interface is created if it does not exists.
 ...
 * @retval error message on error
 */
const char *nfp_ifport_net_dhcp6_up(int port, uint16_t subport_vlan,
				    odp_bool_t if_sp_mgmt);
...
/**
 * Add an IPv6 address on network port interface
 *
 * The function applies to the interface associated to the port (when
 * subport_vlan is NFP_IFPORT_NET_SUBPORT_ITF) or to a VLAN.
 * VLAN interface must exist.
 ...
 * @retval error message on error
 */
const char *nfp_ifport_net_ipv6_addr_add(int port, uint16_t subport_vlan,
					 uint8_t *addr, int masklen);

/**
 * Delete an IPv6 address from network port interface
 *
 * The function applies to the interface associated to the port (when
 * subport_vlan is NFP_IFPORT_NET_SUBPORT_ITF) or to a VLAN.
 *
 * @param port Specify the network port
 * @param subport_vlan Specify the network sub-port.
 * @param addr IPv6 address to delete
 * @param masklen Mask length
 *
 * @retval NULL on success
 * @retval error message on error
 */
const char *nfp_ifport_net_ipv6_addr_del(int port, uint16_t subport_vlan,
					 uint8_t *addr, int masklen);
...
/**
 *  Show IPv4/IPv6 address configured on network ports
 *
 * @param pr NFP printer to use
 */
void nfp_ifport_net_addr_show(nfp_print_t *pr);
