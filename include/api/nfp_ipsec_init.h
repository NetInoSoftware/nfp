 ...
 */
#ifndef __NFP_IPSEC_INIT_H__
#define __NFP_IPSEC_INIT_H__
 ...
 */
#define NFP_IPSEC_MAX_NUM_SA 8
#define NFP_IPSEC_MAX_NUM_SP 8
#define NFP_IPSEC_MAX_INBOUND_SPI 100

/**
 * IPsec initialization parameters
 */
struct nfp_ipsec_param {
	/**
	 ...
	 *
	 * If the value is ODP_QUEUE_INVALID, NFP will create a schedulable
	 * event queue itself if needed.
	 *
	 * If the queue is schedulable and ordered, NFP will make use of its
	 * ordered lock with index 0 (see sched.lock_count queue parameter).
	 *
	 * Ownership of the queue transfers to NFP at nfp_initialize().
	 *
