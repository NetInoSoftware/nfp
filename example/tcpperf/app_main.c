...
static int run_server(void *arg ODP_UNUSED)
{
	 ...
	 *  connections as non-blocking accept is not yet supported in NFP. */
...
static int run_client(void *arg ODP_UNUSED)
{
	...
	/* ToFix: nfp_connect() is not blocking as it should, so spin here
	 * until send succeeds. */
	do {
		uint8_t pkt_buf;
		...
		ret = nfp_send(gbl_args->client_fd, &pkt_buf, 1, 0);
		retry++;
		sleep(1);
...
int main(int argc, char *argv[])
{
	...
	if (nfp_ifport_net_create(gbl_args->appl.if_name,
	...
	if (!(gbl_args->appl.single_thread == 1 && gbl_args->appl.mode == MODE_CLIENT)) {
		 ...
		 * run_server thread to call blocking nfp_accept().
		 */
