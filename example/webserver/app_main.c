...
int main(int argc, char *argv[])
{
	...
	new_workers = nfp_thread_create(thread_tbl, num_workers,
	...
	if (num_workers != new_workers) {
		...
		nfp_stop_processing();
		if (new_workers != -1)
			nfp_thread_join(thread_tbl, new_workers);
		nfp_terminate();
		parse_args_cleanup(&params);
		return EXIT_FAILURE;
	}
	...
	if (nfp_start_webserver_thread(&webserver_pthread,
				       app_init_params.linux_core_id,
				       &webserver_pthread_arg) != 1) {
		NFP_ERR("Error: Failed to create webserver thread");
		nfp_stop_processing();
		nfp_thread_join(thread_tbl, num_workers);
		nfp_terminate();
		parse_args_cleanup(&params);
	...
	nfp_thread_join(thread_tbl, num_workers);
	nfp_thread_join(&webserver_pthread, 1);
...
static void parse_args_cleanup(appl_args_t *appl_args)
{
	...
	if (appl_args->root_dir) {
		free(appl_args->root_dir);
		appl_args->root_dir = NULL;
	}

	if (appl_args->cli_file) {
		free(appl_args->cli_file);
		appl_args->cli_file = NULL;
	}
}
