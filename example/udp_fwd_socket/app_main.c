...
int main(int argc, char *argv[])
{
	...
	/* NFP packet hooks - alternative processing path
	 * app_init_params.pkt_hook[NFP_HOOK_LOCAL_IPv4] = packet_hook_ipv4;
	 * app_init_params.pkt_hook[NFP_HOOK_LOCAL_UDPv4] = packet_hook_udpv4;
	 */
	...
	if (num_workers < 1) {
		NFP_ERR("ERROR: At least 2 cores required.\n");
		nfp_terminate();
		parse_args_cleanup(&params);
		exit(EXIT_FAILURE);
	}
	...
	thread_param.thr_type = ODP_THREAD_WORKER;
	thread_param.description = "timers";
	...
	if (udp_fwd_cfg(params.sock_count, params.laddr, params.raddr)) {
		NFP_ERR("Error: udp_fwd_cleanup failed.");
		nfp_stop_processing();
	}
...
static int parse_args(int argc, char *argv[], appl_args_t *appl_args)
{
	...
	while (res == 0) {
		opt = getopt_long(argc, argv, "+c:i:hf:l:r:s:g",
				  longopts, &long_index);
