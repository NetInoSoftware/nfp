...
Note that utilizing NFP netwrap has some restrictions:
 ...
 - slow path support needs to be disabled in NFP library. Use NFP shared
 library without SP support.
...
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:<nfp_path>/nfp/lib/x86/shared_noSP:<nfp_path>/nfp/lib/.libs
...
the nfp_ifport_net_create() function. This will require the following steps:
...
- calling nfp_ifport_net_create() function for each interface
...
2. tcpperf is a iperf-like NFP test application which can be used for UDP/TCP
